...
int buttonLeftValue = 1;
int buttonDownValue = 1;
int buttonRightValue = 1;


IoT_Error_t rc = NONE_ERROR;

void buttonLeftISR(void)
{
	...
	if (rc == NONE_ERROR) {
		rc = aws_iot_shadow_add_reported(JsonDocumentBuffer, sizeOfJsonDocumentBuffer, 2, &buttonLeftHandler);
		if (rc == NONE_ERROR) {
...
void buttonDownISR(void)
{
	...
	rc = aws_iot_shadow_init_json_document(JsonDocumentBuffer, sizeOfJsonDocumentBuffer);
	if (rc == NONE_ERROR) {
		rc = aws_iot_shadow_add_reported(JsonDocumentBuffer, sizeOfJsonDocumentBuffer, 2, &buttonDownHandler);
		if (rc == NONE_ERROR) {
			rc = aws_iot_finalize_json_document(JsonDocumentBuffer, sizeOfJsonDocumentBuffer);
			if (rc == NONE_ERROR) {
				INFO("Update Shadow: %s", JsonDocumentBuffer);
...
void buttonRightISR(void)
{
	...
	rc = aws_iot_shadow_init_json_document(JsonDocumentBuffer, sizeOfJsonDocumentBuffer);
	if (rc == NONE_ERROR) {
		rc = aws_iot_shadow_add_reported(JsonDocumentBuffer, sizeOfJsonDocumentBuffer, 2, &buttonRightHandler);
		if (rc == NONE_ERROR) {
			rc = aws_iot_finalize_json_document(JsonDocumentBuffer, sizeOfJsonDocumentBuffer);
			if (rc == NONE_ERROR) {
				INFO("Update Shadow: %s", JsonDocumentBuffer);
...
int main(int argc, char** argv) {
	...
	if (wiringPiSetup() < 0){
		fprintf(stdeer, "Unable to setup wiringPi: %s\n", strerror(errno));
		return 1;
	}
	...
	if (wiringPISR(buttonLeft, INT_EDGE_BOTH, &buttonLeftISR) < 0){
		fprintf(stdeer, "Unable to setup ISR: %s\n", strerror(errno));
		return 1;
	}
	if (wiringPISR(buttonDown, INT_EDGE_BOTH, &buttonDownISR) < 0){
		fprintf(stdeer, "Unable to setup ISR: %s\n", strerror(errno));
		return 1;
	}
	if (wiringPISR(buttonRight, INT_EDGE_BOTH, &buttonRightISR) < 0){
		fprintf(stdeer, "Unable to setup ISR: %s\n", strerror(errno));
		return 1;
	}
	...
	buttonDownHandler.type = SHADOW_JSON_FLOAT;
	...
	buttonLeftHandler.type = SHADOW_JSON_FLOAT;
	...
	buttonRightHandler.type = SHADOW_JSON_FLOAT;
	...
	rc = aws_iot_shadow_register_delta(&mqttClient, &buttonLeftHandler, &buttonDownHandler, &buttonRightHandler);
